...
describe('OAuthJweServiceProxy', () => {
	...
	describe('getDcrJweFields', () => {
		it('returns an empty object when no handler is registered (feature off)', async () => {
			...
			await expect(proxy.getDcrJweFields(true, false)).resolves.toEqual({});
		});
		...
		it('returns an empty object when the credential has not opted in', async () => {
			...
			await expect(proxy.getDcrJweFields(false, false)).resolves.toEqual({});
		...
		it('delegates with inlineJwks=false to the registered handler', async () => {
			...
			const result = await proxy.getDcrJweFields(true, false);
		...
		it('delegates with inlineJwks=true to the registered handler', async () => {
			...
			const result = await proxy.getDcrJweFields(true, true);
